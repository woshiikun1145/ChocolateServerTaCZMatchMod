...
public class ClientHandshakeState {
    ...
    public static void onResponse(String serverVersion) {
        ...
        if (compatible) {
            ...
            client.player.sendMessage(Text.literal("§a[模组] 与服务端握手成功（v" + serverVersion + "），功能已启用"), false);
        } else {
            handshakeMessageShown = true;
            client.player.sendMessage(Text.literal(
                    "§c[模组] 与服务端握手失败：版本不匹配（服务端 v" + serverVersion + "），功能已禁用"), false);
        }
    }
    ...
    public static void tick(MinecraftClient client) {
        ...
        if (System.currentTimeMillis() - joinTime > TIMEOUT_MS) {
            ...
            client.player.sendMessage(Text.literal(
                    "§c[模组] 未与服务端握手成功，功能已禁用（服务端可能未安装本模组或版本不匹配）"), false);
        }
    }
